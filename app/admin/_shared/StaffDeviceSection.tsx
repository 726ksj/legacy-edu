...
import { removeStaffDevice } from "./staff-account-actions";
...
export default function StaffDeviceSection({
  role,
  userId,
  devices,
}: {
  ...
  return (
    <div className="flex flex-col gap-3 rounded-lg border border-zinc-200 bg-white p-6">
      ...
      <p className="text-xs text-zinc-500">
        계정당 최대 {MAX_DEVICES_PER_USER}대까지 로그인이 허용됩니다. 더 이상
            ...
            >
              <div className="min-w-0 flex-1">
                ...
                <p className="mt-0.5 text-zinc-400">
                  최근 로그인 {formatDateTime(device.last_seen_at)}
                </p>
              </div>
              <form
                className="shrink-0"
                action={removeStaffDevice.bind(null, role, userId, device.id)}
                onSubmit={(e) => {
