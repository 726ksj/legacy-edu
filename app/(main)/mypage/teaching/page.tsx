...
import { getTeacherCourseIds } from "@/lib/teachers";
...
export default async function TeachingPage() {
  ...
  if (!user) {
    redirect(`/login?redirect=${encodeURIComponent("/mypage/teaching")}`);
  }

  // 관리자는 강좌 배정이 없으니(모든 강좌를 이미 /admin/courses에서
  // 관리할 수 있음) 이 목록 화면 자체가 의미 없어 마이페이지로 돌려보낸다.
  if (isAdmin(user)) {
    redirect("/mypage");
  }
      ...
      {courses.length === 0 ? (
        ...
        <ul className="flex flex-col gap-3">
          {courses.map((course) => (
            <li key={course.id}>
              <Link
                href={`/mypage/teaching/${course.id}`}
              ...
              >
                ...
                <span className="text-sm text-zinc-400">관리하기 →</span>
              </Link>
            </li>
          ))}
        </ul>
