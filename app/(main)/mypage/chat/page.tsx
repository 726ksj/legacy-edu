...
export default async function ChatCoursesPage() {
  const user = await getAuthUser();
  if (!user) {
    redirect(`/login?redirect=${encodeURIComponent("/mypage/chat")}`);
  }

  // 관리자는 강좌 배정이 없으니(모든 강좌를 /admin/chat에서 이미 볼 수
  // 있음) 이 목록 화면 자체가 의미 없어 마이페이지로 돌려보낸다.
  if (isAdmin(user)) {
    redirect("/mypage");
  }
